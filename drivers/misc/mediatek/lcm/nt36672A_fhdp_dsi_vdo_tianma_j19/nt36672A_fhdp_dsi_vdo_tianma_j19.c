...
static unsigned int lcm_compare_id(void)
{
	...
	if (lcm_id0 == 0 && lcm_id1 == 0) {
		LCM_LOGI("[LCM]%s,
		nt36672A moudle id compare success\n", __func__);
		return 1;
	}
...
static unsigned int lcm_ata_check(unsigned char *buffer)
{
	...
	ata_id = buffer_ata[0];
	LCM_LOGI("%s, ata_check, nt36672A ata_id = 0x%x\n", __func__, ata_id);

	if (ata_id == IC_id) {
		LCM_LOGI("%s, nt36672A ata_id compare success\n", __func__);
		return 1;
	}
...
struct LCM_DRIVER nt36672A_fhdp_dsi_vdo_tianma_j19_lcm_drv = {
	.name = "nt36672A_fhdp_dsi_vdo_tianma_j19_lcm_drv",
