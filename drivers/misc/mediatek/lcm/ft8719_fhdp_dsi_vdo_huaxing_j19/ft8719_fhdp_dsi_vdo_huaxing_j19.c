...
static unsigned int lcm_ata_check(unsigned char *buffer)
{
	...
	ata_id = buffer_ata[0];
	LCM_LOGI("%s, ata_check, ft8719 ata_id = 0x%x\n", __func__, ata_id);

	if ((ata_id == IC_id) || (ata_id == 0x9C)) {
		LCM_LOGI("%s, ata_check,ft8719 ata_id compare success\n", __func__);
		return 1;
	}
	return 0;
#else
...
{
	hq_regiser_hw_info(HWID_LCM,
	"incell,vendor:huaxing,IC:ft8719(focaltech)");
}

struct LCM_DRIVER ft8719_fhdp_dsi_vdo_huaxing_j19_lcm_drv = {
	.name = "ft8719_fhdp_dsi_vdo_huaxing_j19_lcm_drv",
