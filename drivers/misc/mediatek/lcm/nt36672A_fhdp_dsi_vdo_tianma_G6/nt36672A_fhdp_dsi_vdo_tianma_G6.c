...
static unsigned int lcm_compare_id(void)
{
	...
	if (id0 == 0x19 && id1 == 0x11) {
		LCM_LOGI("[LCM]%s,nt36672A ic id compare success\n", __func__);
		return 1;
     }
	LCM_LOGI("[LCM]%s,nt36672A ic id compare fail\n", __func__);

    if (lcm_id0 == 1 && lcm_id1 == 1) {
		LCM_LOGI("[LCM]%s,nt36672A moudle id compare success\n", __func__);
		return 1;
     }
	LCM_LOGI("[LCM]%s,nt36672A moudle compare fail\n", __func__);
#endif
     LCM_LOGI(" %s exit\n", __func__);
     return 0;
}
...
static unsigned int lcm_ata_check(unsigned char *buffer)
{
	...
	ata_id = buffer_ata[0];
	printk("%s, ata_check, nt36672A ata_id = 0x%x\n", __func__, ata_id);

	if ((ata_id == IC_id) || (ata_id == 0x9C)) {
		printk("%s, nt36672A ata_id compare success\n", __func__);
		return 1;
	}
...
struct LCM_DRIVER nt36672A_fhdp_dsi_vdo_tianma_lcm_drv_G6 = {
	.name = "nt36672A_fhdp_dsi_vdo_tianma_lcm_drv_G6",
