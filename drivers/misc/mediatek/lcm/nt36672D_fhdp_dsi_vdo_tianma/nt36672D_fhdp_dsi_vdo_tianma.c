...
static unsigned int lcm_compare_id(void)
{
    ...
    if (lcm_id0 == 1 && lcm_id1 == 0) {
         LCM_LOGI("[LCM]%s,nt36672D moudle id compare success\n", __func__);
         return 1;
     }
...
static unsigned int lcm_ata_check(unsigned char *buffer)
{
	...
	ata_id = buffer_ata[0];
	printk("%s, ata_check, nt36672D ata_id = 0x%x\n", __func__, ata_id);

	if ((ata_id == IC_id) || (ata_id == 0x9C)) {
		printk("%s, nt36672D ata_id compare success\n", __func__);
		return 1;
	}
...
{
	hq_regiser_hw_info(HWID_LCM, "incell,vendor:tianma,IC:nt36672D(novatek)");
}

struct LCM_DRIVER nt36672D_fhdp_dsi_vdo_tianma_lcm_drv = {
	.name = "nt36672D_fhdp_dsi_vdo_tianma_lcm_drv",
